...
<div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-auto">
    ...
    <form id="aerialPhotoForm" action="{{ route('aerial-photo.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        <div class="form-group">
            <label for="aerial_photo" class="block text-sm font-medium text-gray-700">Foto Udara<span class="text-red-500">*</span></label>
            <input type="file" name="aerial_photo" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:ring-green-300" accept=".jpg,.jpeg,.png,.tif,.tiff,.geotiff" required>
            <p class="text-xs text-gray-500 mt-1">Format yang didukung: JPG, PNG, TIFF, GeoTIFF. Ukuran maksimal 50MB.</p>
        </div>
        <div class="form-group">
            ...
            <div class="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <input type="text" name="bounds_topleft" id="bounds_topleft" placeholder="contoh: -7.123, 110.456" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:ring-green-300" required>
                </div>
                <div>
                    <label for="bounds_bottomright" class="block text-xs text-gray-600">Bottom Right (Lat, Lng)</label>
                    <input type="text" name="bounds_bottomright" id="bounds_bottomright" placeholder="contoh: -7.234, 110.567" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:ring-green-300" required>
                </div>
            </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="resolution" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:ring-green-300" step="0.01" required>
        </div>
        <div class="form-group">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function() {
        ...
        if (typeof showLoading === 'function') {
            showLoading('Menyimpan foto udara...');
        } else {
            ...
            loadingEl.innerHTML = `
                <div class="bg-white p-4 rounded-lg shadow-lg text-center">
                    ...
                    <p class="text-gray-700">Menyimpan foto udara...</p>
                </div>
            `;
